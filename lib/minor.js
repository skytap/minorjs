...
var cluster         = require('cluster'),
    ...
    jadePlugin      = require('./minorjs_template_jade');
    ...
    listen : function() {
        ...
        return Router.load(this.app, this.options.basePath + "/lib/controllers", this.options)
            .then(function startExpressListening() {
                ...
                self.server = self.app.listen(port, self.options.hostname);

                Logger.profile('Load MinorJS', self.start);

                Logger.info("MinorJS HTTP server listening on " + self.options.hostname + ":" + port);
            })
