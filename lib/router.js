...
var Router = {
    ...
    /**
     * Router._getControllerName(page) -> String
     * - request (String)
     **/
    _getControllerName : function (page) {
        var parts = page.split('/');
        parts.forEach(function (value, index) {
            parts[index] = Humanize.capitalize(value);
        });
        return parts.join('');
    },
    ...
    _handleRequest : function (url, route, controller, request, response, next) {
        var self           = this,
            ...
            controllerName = this._getControllerName(page);
    ...
    _requestFinished : function (controller, request, startTime) {
        controller.emit('request-finished', {
            page   : request.minorjs.page,
            action : request.minorjs.action,
