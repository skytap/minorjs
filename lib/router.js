...
var Router = {
     ...
     **/
    load : function (app, controllerPath, options) {
        var self = this;
    ...
    _handleRequest : function (url, route, startTime, controller, request, response, next) {
        ...
        Filter.run(filters, request, response, next)
            ...
            .timeout(self.options.controllerTimeout)
    ...
    _getMaxRequests: function () {
        ...
        if (typeof(this.maxRequests) === 'undefined') {
            try {
                maxRequests = parseInt(Config.get('max_requests'), 10);
                jitterRange = Math.round(maxRequests * this.MAX_REQUEST_JITTER);
                if (jitterRange > 0) {
                    jitter = Math.floor(Math.random() * (jitterRange * 2 - 1)) - jitterRange;
                } else {
                    jitter = 0;
                }
